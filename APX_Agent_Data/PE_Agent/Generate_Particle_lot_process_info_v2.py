...
# COAT 機台需區分 PR Coat / PI Coat（編號與製程名稱不同，不可混用）
PR_COAT_MACHINES = {'B1_COAT_02', 'B1_COAT_04', 'B1_COAT_07'}
PI_COAT_MACHINES = {'B1_COAT_03', 'B1_COAT_05'}


def _extract_machine_type(machine_id):
    """從 {廠區}_{類型}_{編號} 取出類型前綴，例如 B1_STEP_02 -> STEP。"""
    parts = str(machine_id).split('_')
    if len(parts) >= 2:
        return parts[1].upper()
    return str(machine_id).upper()


def generate_lot_summary(lot_id, group):
    # 提取 PE 經驗標籤 (值為 1 的欄位)
    experience_cols = ['Wait check','PI Coat', 'PI Cure', 'PR Coat', 'PR Exp', 'Sput', 'Etch', 'Final', 'Recon', 'FOUP']
    ...
    for _, row in group.iterrows():
        ...
        record = {
            "stage": row['STAGE'],
            "站點": str(row['OPER']),
            "process": row['PROCESS_CLASSIFICATION'],
            "狀態": status,
            "OOB Rate": oob_rate_display,
            "進出時間": f"{row['TRACK_IN_TIME']} -> {row['TRACK_OUT_TIME']}",
        }
        records.append((machine_id, record))

    # 製程詳細軌跡：以機台為單位的 JSON；同機台多次作業時以 list 保留。
    traj_map = {}
    for machine_id, record in records:
        traj_map.setdefault(machine_id, []).append(record)
    trajectory = {m: (recs[0] if len(recs) == 1 else recs) for m, recs in traj_map.items()}

    # 依 PE 初判嫌疑區，從本 Lot 軌跡擷取對應的實際機台與製程詳細軌跡。
    suspect_machine_map = {}
    for area in suspect_areas:
        prefixes = SUSPECT_AREA_TO_PREFIX.get(area)
        if not prefixes:
            # 表外站點：以站點名稱前綴合理推斷機台類型。
            prefixes = [area.upper()]
        matched = []
        for machine_id, record in records:
            mtype = _extract_machine_type(machine_id)
            if mtype not in prefixes:
                continue
            if mtype == 'COAT':
                mid_upper = machine_id.upper()
                proc = str(record.get('process', ''))
                if area == 'PR Coat':
                    if not (mid_upper in PR_COAT_MACHINES or 'PR Coat' in proc):
                        continue
                elif area == 'PI Coat':
                    if not (mid_upper in PI_COAT_MACHINES or 'PI Coat' in proc):
                        continue
            entry = {"machine": machine_id}
            entry.update(record)
            matched.append(entry)
        if matched:
            suspect_machine_map[area] = matched
        else:
            suspect_machine_map[area] = [
                {"machine": f"{p}_待確認", "備註": "page_content 中無對應機台，待確認"}
                for p in prefixes
            ]

    summary += "#### PE 嫌疑區對應機台:\n"
    summary += json.dumps(suspect_machine_map, ensure_ascii=False, indent=2) + "\n\n"
    summary += "#### 製程詳細軌跡與異常評估:\n"
    summary += json.dumps(trajectory, ensure_ascii=False, indent=2) + "\n"
...
def export_jsonl_to_structured_excel(documents_path, excel_path):
    ...
    with open(documents_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            suspect_marker = "#### PE 嫌疑區對應機台:"
            process_marker = "#### 製程詳細軌跡與異常評估:"

            if suspect_marker in text:
                after_suspect = text.split(suspect_marker, 1)[1]
                if process_marker in after_suspect:
                    suspect_machine = after_suspect.split(process_marker, 1)[0].strip()
                else:
                    suspect_machine = after_suspect.strip()

            if process_marker in text:
                process_detail = text.split(process_marker, 1)[1].strip()
...
# 參數設定 
date_range = (datetime(2026, 7, 3), datetime(2026, 7, 22))
base_path = r"D:\Paticle_OOB_system\Report_APP_BP\APX_Agent_Data\PE_Agent"
# base_path = r"\\Khfs1\8n00$\8N40\Eason\PSN_OOB_共用資料夾\APX_Agent_Data\PE_Agent"
...
output_path = r"\lot_df.csv"
routing_file = base_path + r"\AMD Routing_(Security C).xlsx"
yield_dir    = r"\\khaiotapvp02\AMD\Yield"
local_export_dir = base_path 
...
def main(run_date_range=None):
    ...
    try:
        ...
        if not yield_files:
            ...
            try:
                tmp = pd.read_excel(latest_yield_file, sheet_name="lot merge AEI yield analysis ")
                tmp['Team'] = "AEI"
                tmp = tmp.rename(columns={"F18":"Particle"})
                tmp["Layer"] = "RD" + tmp["Layer"]

                # try:
                #     tmp = pd.read_excel(latest_yield_file, sheet_name="lot merge AEI yield analysis ")
                # except Exception:
                #     tmp = pd.read_excel(latest_yield_file, sheet_name="Single yield particle analysis") # lot Single yield analysis
            ...
            if yield_list:
                ...
                yield_cols = ['LOT_ID', 'Particle', 'Wait check', 'PI Coat', 'PI Cure',
