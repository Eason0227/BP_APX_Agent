...
def get_latest_xlsx(folder: str) -> str:
    try :
        ...
        if not files:
            raise FileNotFoundError(f"找不到任何 xlsx or xlsm檔案：{folder}")    
...
layer_process_map = {
    "RDL1_AEI": [
        6401, 6403, 6405, 6411, 6446, # 原 RDL1_API
    ...
    "RDL2_AEI": [
        6691, 6699, 6700, 6711, 6601, # 原 RDL2_API
    ...
    "RDL3_AEI": [
        6692, 6404, 6707, 6713, 6494, # 原 RDL3_API
...
df_inspection = pd.read_excel(inspection_file, sheet_name="lot merge AEI yield analysis ")
df_inspection['Team'] = "AEI"
...
df_inspection['LOT_RDL_INSPECTION'] = (
    df_inspection['Lot'] + '_' + "RD"+ df_inspection['Layer'] + "_" + df_inspection['Team'].astype(str)
)
df_inspection = df_inspection.rename(columns={"F18":"Particle"})
